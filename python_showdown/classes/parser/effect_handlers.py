...
def _parse_damage(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 2)
    target = parse_pokemon_ident(message.arguments[0])
...
def _parse_heal(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 2)
    target = parse_pokemon_ident(message.arguments[0])
    condition = parse_condition(message.arguments[1])
    return [HealEvent(
...
def _parse_status(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    return [_major_status_event(message, context.source, applied=True)]


def _parse_cure_status(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    return [_major_status_event(message, context.source, applied=False)]


def _parse_weather(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
...
def _parse_forme_change(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 2)
...
def _parse_field(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 1)
...
def _parse_explicit_ability(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    return [_ability_event(message, context.source)]


def _parse_set_hp(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 2)
    target = parse_pokemon_ident(message.arguments[0])
    condition = parse_condition(message.arguments[1])
    return [SetHpEvent(
...
def _parse_set_boost(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
...
def _parse_clear_all_boosts(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 0)
    return [ClearAllBoostsEvent(source=context.source)]


def _parse_clear_negative_boosts(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 0)
    return [ClearNegativeBostsEvent(source=context.source)]


def _parse_item(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 2)
    pokemon = parse_pokemon_ident(message.arguments[0])
...
def _parse_stat_change(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 3)
    target = parse_pokemon_ident(message.arguments[0])
...
def _parse_activation(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
...
def _parse_single_turn(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 2)
...
def _parse_must_recharge(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 1)
    return [MinorStatusEvent(
        context.source,
...
def _parse_transform(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 2)
...
def _parse_prepare(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 2)
    return [MovePrepareEvent(
        pokemon=parse_pokemon_ident(message.arguments[0]),
        move=message.arguments[1],
    )]


def _parse_team_cure(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 1)
...
def _parse_faint(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 1)
    return [MajorStatusEvent(
        context.source,
        parse_pokemon_ident(message.arguments[0]),
...
def _is_failed_stat_change(
    message: ProtocolMessage,
    context: EffectParseContext,
) -> bool:
    return (
        len(message.arguments) >= 2
        and message.arguments[1] in {"boost", "unboost"}
    )


def _parse_failed_stat_change(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 2)
    target = parse_pokemon_ident(message.arguments[0])
...
def _is_ability_start_or_end(message: ProtocolMessage, context: EffectParseContext) -> bool:
    return (
        len(message.arguments) >= 2
...
def _parse_ability_start_or_end(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    return [_ability_event(message, context.source)]


def _is_duplicate_ability_end(message: ProtocolMessage, context: EffectParseContext) -> bool:
    return is_duplicate_silent_ability_end(context.protocol_context, message)


def _discard_effect(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    return []


def _has_effect_name(message: ProtocolMessage, name: str) -> bool:
    return len(message.arguments) >= 2 and message.arguments[1].casefold() == name.casefold()


def _is_type_change(message: ProtocolMessage, context: EffectParseContext) -> bool:
    return _has_effect_name(message, "typechange")


def _parse_type_change(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 3)
    target = parse_pokemon_ident(message.arguments[0])
    types = tuple(
        type_name.strip()
...
def _is_perish_count(message: ProtocolMessage, context: EffectParseContext) -> bool:
    return (
        len(message.arguments) >= 2
...
def _parse_perish_count(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 2)
    target = parse_pokemon_ident(message.arguments[0])
...
def _is_mimic_copy(message: ProtocolMessage, context: EffectParseContext) -> bool:
    return len(message.arguments) >= 3 and _has_effect_name(message, "mimic")


def _parse_mimic_copy(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    return [
        MoveCopiedEvent(
            source=context.source,
            target=parse_pokemon_ident(message.arguments[0]),
...
def _is_volatile_side_condition(message: ProtocolMessage, context: EffectParseContext) -> bool:
    return (
        len(message.arguments) >= 2
...
def _parse_volatile_side_condition(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    target = parse_pokemon_ident(message.arguments[0])
    return [
        SideConditionEvent(
            source=context.source,
...
def _is_minor_status(message: ProtocolMessage, context: EffectParseContext) -> bool:
    return (
        len(message.arguments) >= 2
...
def _parse_minor_status(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    require_arguments(message, 2)
    target = parse_pokemon_ident(message.arguments[0])
    return [MinorStatusEvent(
        parse_effect_source(message, context.source, affected=target),
...
def _parse_unknown_start_end(message: ProtocolMessage, context: EffectParseContext) -> list[BaseEvent]:
    return [unhandled_event(message, context.action_id)]
...
SPECIAL_EFFECT_RULES: dict[str, tuple[EffectRule, ...]] = {
    "-fail": (
        EffectRule(_is_failed_stat_change, _parse_failed_stat_change),
    ),
    "-start": (
